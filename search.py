import re , csv, os, time, requests, win32clipboard
...
class Search(object):
    def __init__(self):
        self.language = 'en'
        self.api_key = data['key'][0]
        self.token = data['token'][0]
        self.google_key = 'f7170e885e46770e0'
        self.nums = 3
        self.error = 0
        self.old = []
        
def getAnswer(pageUrl,question,object):
    # print('\n')
    ...
    if title == 'Page Unavailable':
        ...
        scenario_3(question, soup,object)
    elif answer == "...":
        pass
...
def scenario_3(desired_question, soup, object):
    print("Scenario 3")
...
def findAnswers(question,object):
    possibleAnswers = []
    ...
    index = 1
    result = ""
    for j in google_search(question, object.api_key, object.token, num=object.nums):
        print("Search # ", index)
        index += 1
        getAnswer(j,question,object)
        print("\n")
...
def main(object):
    while True:
        ...
        # print(len(old))
        if data not in object.old and data != "none":
            object.old.append(data)
            print(data)
            # play(data)
            # search(data)
            ...
            if found:
                ...
                play("searching")
                findAnswers(data,object)
                if object.error >= object.nums:
                    print("Errors: ", object.error)
                    no = "Not Found"
                    print("Not Found")
                    play(no)
                    object.error = 0
        else:
            print("Waiting for new clipboard data.")
            time.sleep(3)
            if len(object.old) > 3:
                object.old.pop(0)
